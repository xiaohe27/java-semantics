...
cat max3.in | krun $1 --directory "$SCRIPTPATH/../src/prep" --parser "kj-parse-aggreg.sh" --output kast > $OUT
...
cat max3.in | krun $OUT --directory "$SCRIPTPATH/../src/exec" -cMainClass="ListItem(\"$ClsName\")" -cDissolveAllExceptOut="true" --parser "cat"
